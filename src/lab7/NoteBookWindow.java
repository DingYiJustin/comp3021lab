...
import lab7.Folder;
import lab7.Note;
import lab7.NoteBook;
import lab7.TextNote;
...
public class NoteBookWindow extends Application {
	...
	private HBox addHBox() {
		...
		hbox.getChildren().addAll(buttonLoad, buttonSave, search, searchField, 
				buttonSearch, buttonClearSearch);
	...
	private VBox addVBox() {
		...
		titleslistView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Object>() {
			...
			public void changed(ObservableValue ov, Object t, Object t1) {
				...
				// textAreNote
				String content = "";
				Folder current = new Folder(currentFolder);
				for(Folder folder: noteBook.getFolders()) {
					if(folder.equals(current)) {
						for(Note note: folder.getNotes()) {
							if(note.equals(new Note(title))) {
	...
	private void updateListView() {
		...
		// currentFolder
		if(currentSearch == null && currentFolder != null) {
			Folder current = new Folder(currentFolder);
			for(Folder folder: noteBook.getFolders()) {
				if(folder.equals(current)) {
					for(Note note: folder.getNotes()) {
		...
		else if(currentSearch != null && currentFolder != null) {
			Folder current = new Folder(currentFolder);
			for(Folder folder: noteBook.getFolders()) {
				if(folder.equals(current)) {
					List<Note> notelist = folder.searchNotes(currentSearch);
