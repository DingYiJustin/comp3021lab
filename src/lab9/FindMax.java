...
public class FindMax {
	...
	public void printMax() throws InterruptedException {
		// this is a single threaded version
		int[] max3 = new int[3]; 
		Thread t1 = new Thread(()->{max3[0]=findMax(0,29);});
		Thread t2 = new Thread(()->{max3[1]=findMax(30,59);});
		Thread t3 = new Thread(()->{max3[2]=findMax(60,89);});
		t1.start();
		t2.start();
		t3.start();
		t1.join();
		...
		int max12 = (max3[0]>max3[1] ? max3[0]:max3[1]);
		int max = max12>max3[2]? max12:max3[2];
